...
export default function Body(props) {
    ...
    const handleFilterText = () => {
        ...
        // Replace the matched word with a highlighted version using <span>
    ...
    return (
        <>
            <div className='container' style={{backgroundColor: props.mode === 'dark' ? '#434343' : 'white', color: props.mode === 'dark' ? 'white' : 'black'}} >
                <h1>{props.heading}</h1> 
                ...
                <button disabled={text.length===0} className="btn btn-primary mx-2 my-1" onClick={handleOnClick} >
                    Convert To Uppercase
                </button>
                {/* Button to trigger the handleOnClick function */}
                <button disabled={text.length===0} className="btn btn-primary mx-2 my-1" onClick={handleOnClick1} >
                    Convert To Lowercase
                </button>
                <button disabled={text.length===0} className="btn btn-primary mx-2 my-1" onClick={handleClearText} >
                    Clear Text
                </button>
                <button disabled={text.length===0} className="btn btn-primary my-2 my-1" onClick={handleCopyText}>
                        Copy Text
                </button>   
                <div className="my-3" style={{backgroundColor: props.mode === 'dark' ? '#434343' : 'white', color: props.mode === 'dark' ? 'white' : 'black'}}>
                    <input 
                        type="text" 
                        className="form-control" 
                        value={searchWord} 
            ...
            <div className="container my-3" style={{backgroundColor: props.mode === 'dark' ? '#434343' : 'white', color: props.mode === 'dark' ? 'white' : 'black'}}>
                <h1>Your Text Summary</h1>
                <p>Number Of Words : {text.trim() ? text.trim().split(/\s+/).length : 0}</p>
                <p>Number Of Characters : {text.length}</p>
                <p>Average Time Taken By Reader to Read The Text is: {text.trim() ? (text.trim().split(/\s+/).length * 0.008).toFixed(2) : "0"} Minutes </p>
                <h2>Filtered Text (Highlighted)</h2>
                <p 
                    dangerouslySetInnerHTML={{ __html: filteredText || 'No matching words found or no word entered.' }}>
                </p>
                {/* Display the filtered text with highlighted words using dangerouslySetInnerHTML */}
            </div>
        </>
    );
}
